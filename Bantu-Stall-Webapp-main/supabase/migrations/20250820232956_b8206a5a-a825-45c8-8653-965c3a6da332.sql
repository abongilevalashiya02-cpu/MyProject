...
-- Add enhanced security triggers for audit logging
...
-- Create security event logging for suspicious activity
CREATE OR REPLACE FUNCTION public.detect_suspicious_activity()
RETURNS TRIGGER AS $$
DECLARE
    recent_count integer;
BEGIN
    -- Count recent access attempts by same user
    SELECT COUNT(*) INTO recent_count
    FROM audit_logs
    WHERE user_id = auth.uid()
    AND resource = TG_TABLE_NAME
    AND created_at > now() - interval '1 minute';
    
    -- Log security event if too many rapid accesses
...
-- Add suspicious activity detection to sensitive tables
DROP TRIGGER IF EXISTS detect_suspicious_clients_access ON public.clients;
CREATE TRIGGER detect_suspicious_clients_access
    AFTER SELECT
    ON public.clients
    FOR EACH ROW
...
VALUES ('security_scan', 'sensitive_data_access', 0, now())
