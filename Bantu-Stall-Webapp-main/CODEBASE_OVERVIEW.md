...
- `market-prices`: Authenticated REST endpoint that estimates market prices for experiential travel & education packages. Uses `service_catalog` medians per category and builds historical trends from `quotation_requests`. Supports parameters for group size, nights, occupancy, meals per day, hours per day, currency (with FX conversion from ZAR), and returns VAT-inclusive totals plus per-category breakdown.
